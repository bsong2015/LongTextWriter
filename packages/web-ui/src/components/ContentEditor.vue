...
watch(() => [props.generatedContent, props.project] as const, ([newContent, newProject]) => {
  if (newContent && newContent.chapters && newContent.chapters.length > 0) {
    treeData.value = convertContentToTreeData(newContent);
  } else if (newProject && (newProject.type === 'book' || newProject.type === 'series') && newProject.outline) {
