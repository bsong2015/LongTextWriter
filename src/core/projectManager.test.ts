...
import { BookOutline, GeneratedContent } from '../types';
...
describe('createProject', () => {
  ...
  it('should throw an error for an invalid project type', () => {
    ...
    expect(() => createProject(projectData)).toThrow('Invalid project type.');
...
describe('getProjectList', () => {
  ...
  it('should list book and series projects with correct status', () => {
    ...
    const templatedOutputPath = path.resolve(templatedProjectPath, 'output', `${templatedProjectName}.md`);

    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === PROJECTS_DIR) return true;
      if (p === bookProjectPath) return true;
      if (p === seriesProjectPath) return true;
      if (p === templatedProjectPath) return true;
      if (p === bookProjectJsonPath) return true;
      if (p === bookGeneratedJsonPath) return true; // Book project has generated content
      if (p === seriesProjectJsonPath) return true;
      if (p === seriesGeneratedJsonPath) return false; // Series project has no generated content yet
      if (p === templatedProjectJsonPath) return true;
      if (p === templatedOutputPath) return true; // Templated project has output
      return false;
    });
      ...
      if (p === bookProjectJsonPath) {
        return JSON.stringify({
          ...
          outline: {
            title: 'Book Outline',
            chapters: [
              { title: 'Chapter 1', articles: [{ title: 'a1' }, { title: 'a2' }] },
              { title: 'Chapter 2', articles: [{ title: 'a3' }] },
            ],
          },
        });
      }
          ...
          chapters: [
            {
              ...
              articles: [
                ...
                { title: 'a2', status: 'pending' },
              ],
            },
            {
              title: 'Chapter 2',
              articles: [
                { title: 'a3', status: 'done' },
              ],
            },
          ],
        });
      ...
      if (p === seriesProjectJsonPath) {
        return JSON.stringify({
          ...
          outline: {
            ...
            chapters: [
              { title: 'Part 1', articles: [{ title: 's1' }] },
            ],
          },
        });
      }
    ...
    expect(bookProject).toBeDefined();
    ...
    expect(seriesProject).toBeDefined();
    ...
    expect(templatedProject).toBeDefined();
    expect(templatedProject?.type).toBe('templated');
    expect(templatedProject?.status).toEqual({ type: 'icon', value: 'completed' });
  });
...
describe('getProjectDetails', () => {
  ...
  it('should return details for a templated project with completed status', () => {
    const projectName = 'detailed-templated-project';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const templatedOutputPath = path.resolve(projectPath, 'output', `${projectName}.md`);

    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) return true;
      if (p === templatedOutputPath) return true;
      return false;
    });
    ...
    expect(details.status).toEqual({ type: 'icon', value: 'completed' });
    expect(details).toHaveProperty('createdAt');
  });

  it('should return details for a templated project with pending status', () => {
    const projectName = 'detailed-templated-project-pending';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const templatedOutputPath = path.resolve(projectPath, 'output', `${projectName}.md`);
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) return true;
      if (p === templatedOutputPath) return false; // Output file does not exist
      return false;
    });
    ...
    expect(details.name).toBe(projectName);
    expect(details.type).toBe('templated');
    expect(details.status).toEqual({ type: 'icon', value: 'pending' });
    expect(details).toHaveProperty('createdAt');
  });
...
describe('getProjectOutline', () => {
  ...
  it('should return the outline for a book project', () => {
    const projectName = 'book-with-outline';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const mockOutline: BookOutline = {
      title: 'My Book Outline',
      chapters: [{ title: 'Chapter 1', articles: [{ title: 'Article 1.1' }] }],
    };
  ...
  it('should return undefined if a book project has no outline', () => {
  ...
  it('should throw an error for unsupported project type', () => {
    const projectName = 'templated-project-no-outline';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    ...
    expect(() => getProjectOutline(projectName)).toThrow(`Outline not supported for project type templated.`);
  });
...
describe('generateProjectOutline', () => {
  ...
  beforeEach(() => {
    ...
    (fs.existsSync as jest.Mock).mockReturnValue(false);
    (fs.readFileSync as jest.Mock).mockReturnValue('');
    process.env.OPENAI_API_KEY = 'test-api-key'; // Ensure API key is set for tests
    process.env.MOCK_LLM = 'true'; // Set MOCK_LLM to true
  });

  afterEach(() => {
    delete process.env.OPENAI_API_KEY;
    delete process.env.MOCK_LLM; // Clean up MOCK_LLM
  });
  ...
  it('should overwrite an existing outline if overwrite is true', async () => {
    const projectName = 'book-to-overwrite-outline';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const oldOutline: BookOutline = {
      title: 'Old Outline',
      chapters: [{ title: 'Old Chapter', articles: [{ title: 'Old Article' }] }],
    };
    ...
    const result = await generateProjectOutline(projectName, true);
  ...
  it('should throw an error for unsupported project type', async () => {
    ...
    await expect(generateProjectOutline(projectName)).rejects.toThrow(`Outline not supported for project type templated.`);
  });
...
describe('saveProjectOutline', () => {
  ...
  it('should save a new outline for a book project', async () => {
    const projectName = 'book-to-save-outline';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
  ...
  it('should throw an error for unsupported project type', async () => {
    const projectName = 'templated-project-save-outline';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    ...
    await expect(saveProjectOutline(projectName, mockOutline)).rejects.toThrow(`Project ${projectName} of type templated does not support outlines.`);
  });
...
describe('startContentGeneration', () => {
  ...
  it('should successfully generate content for a book project', async () => {
    const projectName = 'book-project-generate';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const generatedJsonPath = path.resolve(projectPath, 'generated.json');
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) return true;
      // Initially, generated.json does not exist
      return false;
    });
    ...
    // Expect writeFileSync to be called multiple times:
    // 1. For initial generated.json creation
    // 2. For each article status update (writing, done)
    // 3. For each chapter summary update
    // The exact count depends on the internal logic, but we can check the content of the final write
    const writeFileSyncCalls = (fs.writeFileSync as jest.Mock).mock.calls;
    expect(writeFileSyncCalls.length).toBeGreaterThan(0);

    // Verify the final generated.json content
    const finalGeneratedContentCall = writeFileSyncCalls.find(call => call[0] === generatedJsonPath && call[1].includes('done'));
    expect(finalGeneratedContentCall).toBeDefined();

    const finalGeneratedContent = JSON.parse(finalGeneratedContentCall[1]);
    expect(finalGeneratedContent.title).toBe('My Book Outline');
    expect(finalGeneratedContent.chapters[0].articles[0].status).toBe('done');
  });

  it('should successfully generate content for a templated project', async () => {
    const projectName = 'templated-project-generate';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const sourceFilePath = path.resolve(projectPath, 'src', 'data.txt');
    const templateFilePath = path.resolve(projectPath, 'templates', 'template.md');
    const outputFilePath = path.resolve(projectPath, 'output', `${projectName}.md`);
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) return true;
      if (p === sourceFilePath) return true;
      if (p === templateFilePath) return true;
      return false;
    });
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((p) => {
      ...
      if (p === sourceFilePath) {
        return 'Source content for templated project.';
      }
      if (p === templateFilePath) {
        return '# My Template\n\n<AI_FILL: Write an introduction based on source content>\n\nConclusion.';
      }
      return '';
    });
    ...
    // Expect mkdirSync for output directory and writeFileSync for the output file
    expect(fs.mkdirSync).toHaveBeenCalledWith(path.resolve(projectPath, 'output'), { recursive: true });    
  });
  ...
  it('should throw an error if book/series project has no outline', async () => {
  ...
  it('should throw an error if writeFileSync fails during book content generation', async () => {
    const projectName = 'book-write-fail';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const generatedJsonPath = path.resolve(projectPath, 'generated.json');
    ...
    (fs.writeFileSync as jest.Mock).mockImplementation((filePath, content) => {
      // Allow the first write (initial generated.json) but fail subsequent writes
      if (filePath === generatedJsonPath && writeCount === 0) {
        writeCount++;
        return;
      } else if (filePath === generatedJsonPath && writeCount > 0) {
        throw new Error('Simulated write error');
      }
    });

    await expect(startContentGeneration(projectName)).rejects.toThrow(`Simulated write error`);
  });

  it('should throw an error if source file not found for templated project', async () => {
    const projectName = 'templated-source-not-found';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const sourceFilePath = path.resolve(projectPath, 'src', 'data.txt');
    const templateFilePath = path.resolve(projectPath, 'templates', 'template.md');
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) return true;
      if (p === sourceFilePath) return false; // Source file does not exist
      if (p === templateFilePath) return true;
      return false;
    });
    ...
    await expect(startContentGeneration(projectName)).rejects.toThrow(`Source file not found: ${sourceFilePath}.`);
  });

  it('should throw an error if template file not found for templated project', async () => {
    const projectName = 'templated-template-not-found';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const sourceFilePath = path.resolve(projectPath, 'src', 'data.txt');
    const templateFilePath = path.resolve(projectPath, 'templates', 'template.md');
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) return true;
      if (p === sourceFilePath) return true;
      if (p === templateFilePath) return false; // Template file does not exist
      return false;
    });
    ...
    await expect(startContentGeneration(projectName)).rejects.toThrow(`Template file not found: ${templateFilePath}.`);
  });

  it('should return success message if no AI_FILL placeholders found in template', async () => {
    const projectName = 'templated-no-ai-fill';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const sourceFilePath = path.resolve(projectPath, 'src', 'data.txt');
    const templateFilePath = path.resolve(projectPath, 'templates', 'template.md');
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((p) => {
      ...
      if (p === templateFilePath) {
        return '# My Template\n\nNo AI fill here.'; // No AI_FILL placeholder
      }
      return '';
    });

    const result = await startContentGeneration(projectName);
    expect(result).toEqual({ message: 'No AI_FILL placeholders found in template.' });
    expect(fs.writeFileSync).not.toHaveBeenCalledWith(outputFilePath, expect.any(String), 'utf-8');
  });
});
...
describe('publishProject', () => {
  ...
  it('should successfully publish a series project (single-markdown)', async () => {
    const projectName = 'series-to-publish-single';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const generatedJsonPath = path.resolve(projectPath, 'generated.json');
    const expectedOutputPath = path.resolve(projectPath, 'output', `${projectName}.md`);
  ...
  it('should successfully publish a templated project', async () => {
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((p) => {
      ...
      if (p === templatedOutputPath) {
        return '# Templated Content Example\n\nThis is the content from the templated output.';
      }
      return '';
    });
  ...
  it('should throw an error if no generated content found for book/series project', async () => {
    ...
    const mockProjectJson = {
      ...
      type: 'book',
      createdAt: new Date().toISOString(),
      idea: { language: 'en', summary: 'Outline summary', prompt: 'Outline prompt' }, // Added idea
      outline: { title: 'Outline', chapters: [] },
    };

    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      return p === projectJsonPath; // project.json exists, generated.json does not
    });
    (fs.readFileSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) {
        return JSON.stringify(mockProjectJson);
  ...
  it('should throw an error if no templated output file found for templated project', async () => {
    const projectName = 'templated-no-output-file';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const templatedOutputPath = path.resolve(projectPath, 'output', `${projectName}.md`);
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      return p === projectJsonPath; // project.json exists, templated output does not
    });
    (fs.readFileSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) {
        return JSON.stringify(mockProjectJson);
    ...
    await expect(publishProject(projectName, 'single-markdown')).rejects.toThrow(`No templated output found for project ${projectName}.`);
    expect(fs.writeFileSync).not.toHaveBeenCalled();
  });
  ...
  it('should throw an error if writeFileSync fails during publication', async () => {
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      return p === projectJsonPath || p === generatedJsonPath || p === path.dirname(expectedOutputPath);
    });
    (fs.readFileSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) {
        return JSON.stringify(mockProjectJson);
      ...
      if (p === generatedJsonPath) {
        return JSON.stringify({ title: 'My Book', chapters: [] });
      }
      return '';
    });
    (fs.writeFileSync as jest.Mock).mockImplementation(() => {
  ...
  it('should create output directory if it does not exist', async () => {
    const projectName = 'book-create-dir';
    const projectPath = path.resolve(PROJECTS_DIR, projectName);
    const projectJsonPath = path.resolve(projectPath, 'project.json');
    const generatedJsonPath = path.resolve(projectPath, 'generated.json');
    const expectedOutputPath = path.resolve(projectPath, 'output', `${projectName}.md`);
    ...
    // Mock existsSync to return false for the output directory initially
    (fs.existsSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) return true;
      if (p === generatedJsonPath) return true;
      if (p === path.dirname(expectedOutputPath)) return false; // Output directory does not exist
      return false;
    });
    (fs.readFileSync as jest.Mock).mockImplementation((p) => {
      if (p === projectJsonPath) {
    ...
    await publishProject(projectName, 'single-markdown');
    expect(fs.mkdirSync).toHaveBeenCalledWith(path.dirname(expectedOutputPath), { recursive: true });
    expect(fs.writeFileSync).toHaveBeenCalled(); // Ensure writeFileSync is still called
  });
});
